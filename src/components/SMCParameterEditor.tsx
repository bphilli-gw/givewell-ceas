...
import type { SMCCountryData, SMCMainCEAResult } from '../model/smc-types';
import { calculateSMCMainCEA } from '../model/smc-cea';
...
const EDITABLE_PARAMS: ParamDef[] = [
  ...
  { key: 'cost_per_cycle', label: 'Cost per Cycle ($)', path: ['cost', 'cost_per_cycle'], format: 'currency', step: 0.1, min: 0.01 },
  ...
  { key: 'discount_rate', label: 'Discount Rate', path: ['outcomes', 'discount_rate'], format: 'percent', step: 0.005, min: 0, max: 0.2 },
  { key: 'internal_validity_adj', label: 'Internal Validity Adj', path: ['outcomes', 'internal_validity_adj'], format: 'percent', step: 0.01 },
  { key: 'external_validity_adj', label: 'External Validity Adj', path: ['outcomes', 'external_validity_adj'], format: 'percent', step: 0.01 },
  { key: 'mortality_to_incidence_ratio', label: 'Mortality/Incidence Ratio', path: ['outcomes', 'mortality_to_incidence_ratio'], format: 'number', step: 0.1, min: 0 },
];
...
interface Props {
  country: SMCCountryData;
  onRecalculate: (result: SMCMainCEAResult | null) => void;
}

export default function SMCParameterEditor({ country, onRecalculate }: Props) {
  ...
  const handleChange = useCallback(
    (param: ParamDef, rawValue: string) => {
      ...
      const newInputs = setNestedValue(inputs, param.path, numValue);
      setInputs(newInputs);
      setModified((prev) => new Set(prev).add(param.key));

      const result = calculateSMCMainCEA(
        newInputs as unknown as SMCCountryData['inputs'],
      );
      onRecalculate(result);
    },
    [inputs, onRecalculate],
  );
