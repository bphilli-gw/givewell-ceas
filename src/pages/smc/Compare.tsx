...
interface ComparisonMetric {
  key: string;
  label: string;
  getValue: (c: SMCCountryData) => number;
...
const METRICS: ComparisonMetric[] = [
  ...
  },
  {
    key: 'cost_per_life',
    label: 'Cost per Life',
    getValue: (c) => c.results.cost_per_counterfactual_life,
    format: 'currency',
  },
  {
    key: 'children_covered',
    label: 'Children Covered',
    getValue: (c) => c.results.total_children_covered,
    format: 'number',
  },
  {
    key: 'cases_averted',
    label: 'Total Cases Averted',
    getValue: (c) => c.results.total_cases_averted,
    format: 'number',
  },
];
...
export default function SMCCompare() {
  ...
  const selectedCountries = useMemo(() => {
    ...
    return selected
      ...
      .filter(Boolean) as SMCCountryData[];
  }, [data, selected]);
  ...
  const radarData = useMemo(() => {
    ...
    const radarMetrics = [METRICS[0], METRICS[1], METRICS[4], METRICS[5]];
