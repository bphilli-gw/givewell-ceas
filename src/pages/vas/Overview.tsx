...
export default function VASOverview() {
  const { data, loading, error } = useVASCountryData();
  const navigate = useNavigate();

  if (loading) return <div className="loading">Loading VAS data...</div>;
  if (error) return <div className="error">Error loading data: {error}</div>;
  if (!data) return null;

  const countries = data.countries;
  const avgCE = countries.reduce((sum, c) => sum + c.results.ce_multiple, 0) / countries.length;
  ...
  const maxP95 = Math.max(
    ...countries.map((c) => c.monte_carlo?.summary.p95 ?? c.results.ce_multiple)
  );

  return (
    <div className="page overview-page">
      <h1>VAS Cost-Effectiveness Analysis</h1>
      ...
      <div className="table-container">
        <table className="results-table">
          ...
          <tbody>
            {countries.map((c: VASCountryData, i: number) => {
              const mc = c.monte_carlo;
              const ce = c.results.ce_multiple;
              return (
                <tr
                  key={c.id}
                  className="clickable-row"
                  onClick={() => navigate(`/vas/country/${c.id}`)}
                >
                  ...
                  <td className="num">{fmtCurrency(c.results.cost_per_life_saved)}</td>
                  <td className="num">{fmtInt(c.results.children_covered)}</td>
                </tr>
              );
            })}
          </tbody>
